...
b_f_fr_gdd <- ggplot(plot_df_all_gdd, aes(
  ...
  labs(
    ...
    y = "Onset (GDD5)",
    title = "Effect of transplantation and warming on onset GDD 12h",
...
# ggsave(filename = "Output/Onset/GDD_Transplantation_Onset_bud_flower_fruit_seeds_tms_all_treat_sign_12h.png", 
#        plot = b_f_fr_gdd, width = 18, height = 10, units = "in")




sig_tab_onset_wide_gdd <- letters_all_gdd |>
  mutate(
    estimate_ci = sprintf(
      "%.1f (%.1f, %.1f)",
      emmean,
  ...
  tab_header(
    title = md("**Predicted mean onset GDD**"),
...
sig_tab_onset_wide_gdd

#gtsave(sig_tab_onset_wide_gdd, "Output/Onset/Onset_GDD_signif_per_stage_wide_NOR_CHE.docx")

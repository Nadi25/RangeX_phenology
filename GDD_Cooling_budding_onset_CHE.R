...
## Date:      08.09.25
## Author:    Nadine Arzt
## Purpose:   Effect of cooling on budding onset CHE - with GDD
...
budding_onset_gdd_ambi_che <- phenology_with_gdd_che_ambi |> 
...
m_onset_gdd_ambi_che_bud <- lmerTest::lmer(onset ~ site * treat_competition + (1|species) + (1|block_ID),
                                       data = budding_onset_gdd_ambi_che)
summary(m_onset_gdd_ambi_che_bud)


model_performance(m_onset_gdd_ambi_che_bud)
check_collinearity(m_onset_gdd_ambi_che_bud)
check_model(m_onset_gdd_ambi_che_bud)


# plot point with residuals for effect of warming on flo onset -------------
# estimated marginal means
emm_nor_gdd_ambi_che_bud <- emmeans(m_onset_gdd_ambi_che_bud, ~ site | treat_competition)

contr_nor_gdd_ambi_che_bud <- contrast(emm_nor_gdd_ambi_che_bud, method = list("hi - lo" = c(1, -1)))

# using summary keeps the p-values
contrast_df_nor_gdd_ambi_che_bud <- as.data.frame(summary(contr_nor_gdd_ambi_che_bud, infer = TRUE))


# with raw data points ----------------------------------------------------
# compute mean onset per treatment × group
onset_means_cool_gdd_che_bud <- budding_onset_gdd_ambi_che |>
...
che_delta_raw_cool_gdd_bud<- ggplot() +
  ...
  # significance labels
  geom_text(data = contrast_df_nor_gdd_ambi_che_bud,
  ...
  labs(x = "Biotic interactions",
       y = "Δ GDD shifted _budding onset (high - low)",
       title = "Effect of cooling on budding onset CHE") +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 20),
        axis.text.y = element_text(size = 20))+
  scale_color_manual(values = c("#528B8B", "#CD950C"))
che_delta_raw_cool_gdd_bud


ggsave(filename = "Output/Onset/GDD_cooling_budding_onset_CHE.png",
       plot = che_delta_raw_cool_gdd_bud, width = 10, height = 8, units = "in")

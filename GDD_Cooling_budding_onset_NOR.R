...
# RangeX phenology effect of cooling on budding NOR ------------
...
## Date:      26.03.26
## Author:    Nadine Arzt
## Purpose:   Effect of cooling on budding onset NOR - with GDD
...
budding_onset_gdd_ambi <- phenology_with_gdd_ambi |> 
...
m_onset_gdd_ambi_bud <- lmerTest::lmer(onset ~ site * treat_competition + (1|species) + (1|block_ID),
...
# block has 0 random effect?
# why? should I take it out then?

# plot point with residuals for effect of warming on flo onset -------------
# estimated marginal means
emm_nor_gdd_ambi_bud <- emmeans(m_onset_gdd_ambi_bud, ~ site | treat_competition)

contr_nor_gdd_ambi_bud  <- contrast(emm_nor_gdd_ambi_bud , method = list("hi - lo" = c(1, -1)))

# using summary keeps the p-values
contrast_df_nor_gdd_ambi_bud <- as.data.frame(summary(contr_nor_gdd_ambi_bud , infer = TRUE))


# with raw data points ----------------------------------------------------
# compute mean onset per treatment × group
onset_means_cool_gdd_bud  <- budding_onset_gdd_ambi |>
...
nor_che_delta_raw_cool_gdd_bud  <- ggplot() +
  ...
  # significance labels
  geom_text(data = contrast_df_nor_gdd_ambi_bud,
  ...
  labs(x = "Biotic interactions",
       y = "Δ GDD shifted budding onset (high - low)",
       title = "Effect of cooling on budding onset NOR") +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 20),
        axis.text.y = element_text(size = 20))+
  scale_color_manual(values = c("#528B8B", "#CD950C"))
nor_che_delta_raw_cool_gdd_bud


# ggsave(filename = "Output/Onset/GDD_cooling_budding_onset_NOR.png", 
#        plot = nor_che_delta_raw_cool_gdd_bud, width = 10, height = 8, units = "in")

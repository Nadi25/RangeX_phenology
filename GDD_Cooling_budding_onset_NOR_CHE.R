...
m_onset_gdd_ambi_bud <- lmerTest::lmer(onset ~ region * site * treat_competition + 
...
check_model(m_onset_gdd_ambi_bud)


# Plot with interactions --------------------------------------------------

emm_full_gdd_bud <- emmeans(
  m_onset_gdd_ambi_bud,
  ~ site * treat_competition | region)


contr_cooling_gdd_bud <- contrast(
  emm_full_gdd_bud,
...
df_cooling_gdd_bud <- as.data.frame(summary(contr_cooling_gdd_bud, infer = TRUE)) |>
  mutate(effect = "Transplantation",
         group = treat_competition)


contr_comp_gdd_bud <- contrast(
  emm_full_gdd_bud,
...
df_comp_gdd_bud <- as.data.frame(summary(contr_comp_gdd_bud, infer = TRUE)) |>
  mutate(effect = "Biotic interactions",
         group = site)



contr_interaction_gdd_bud <- contrast(
  emm_full_gdd_bud,
  interaction = "pairwise"
)

df_interaction_gdd_bud <- as.data.frame(summary(contr_interaction_gdd_bud, infer = TRUE)) |>
...
df_cooling_gdd_bud     <- add_stars(df_cooling_gdd_bud)
df_comp_gdd_bud        <- add_stars(df_comp_gdd_bud)
df_interaction_gdd_bud <- add_stars(df_interaction_gdd_bud)

plot_df_gdd_bud <- bind_rows(
  df_cooling_gdd_bud     |> select(region, effect, group, estimate, lower.CL, upper.CL, stars),
  df_comp_gdd_bud        |> select(region, effect, group, estimate, lower.CL, upper.CL, stars),
  df_interaction_gdd_bud |> select(region, effect, group, estimate, lower.CL, upper.CL, stars)
)

plot_df_gdd_bud <- plot_df_gdd_bud |>
  mutate(effect = factor(effect,
                         levels = c("Transplantation",
                                    "Biotic interactions",
                                    "Interaction")))
#
delta_raw_gdd_bud <- budding_onset_gdd_ambi |>
...
cooling_bud_interaction_gdd_flower_nor_che <- ggplot(plot_df_gdd_bud,
  ...
  labs(x = NULL,
       y = "Shifted onset (GDD)",
       color = "Condition")
cooling_bud_interaction_gdd_flower_nor_che



# ggsave(filename = "Output/Onset/GDD_cooling_budding_onset_NOR_CHE.png", 
#        plot = cooling_bud_interaction_gdd_flower_nor_che,

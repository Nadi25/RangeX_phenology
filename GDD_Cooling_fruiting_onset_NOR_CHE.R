...
m_onset_gdd_ambi_fruit <- lmerTest::lmer(onset ~ region * site * treat_competition + 
...
emm_full_gdd_fruit <- emmeans(
  m_onset_gdd_ambi_fruit,
  ~ site * treat_competition | region)


contr_cooling_gdd_fruit <- contrast(
  emm_full_gdd_fruit,
...
df_cooling_gdd_fruit <- as.data.frame(summary(contr_cooling_gdd_fruit, infer = TRUE)) |>
  mutate(effect = "Transplantation",
         group = treat_competition)


contr_comp_gdd_fruit <- contrast(
  emm_full_gdd_fruit,
...
df_comp_gdd_fruit <- as.data.frame(summary(contr_comp_gdd_fruit, infer = TRUE)) |>
  mutate(effect = "Biotic interactions",
         group = site)



contr_interaction_gdd_fruit <- contrast(
  emm_full_gdd_fruit,
  interaction = "pairwise"
)

df_interaction_gdd_fruit <- as.data.frame(summary(contr_interaction_gdd_fruit, infer = TRUE)) |>
...
df_cooling_gdd_fruit     <- add_stars(df_cooling_gdd_fruit)
df_comp_gdd_fruit        <- add_stars(df_comp_gdd_fruit)
df_interaction_gdd_fruit <- add_stars(df_interaction_gdd_fruit)

plot_df_gdd_fruit <- bind_rows(
  df_cooling_gdd_fruit     |> select(region, effect, group, estimate, lower.CL, upper.CL, stars),
  df_comp_gdd_fruit        |> select(region, effect, group, estimate, lower.CL, upper.CL, stars),
  df_interaction_gdd_fruit |> select(region, effect, group, estimate, lower.CL, upper.CL, stars)
)

plot_df_gdd_fruit <- plot_df_gdd_fruit |>
  mutate(effect = factor(effect,
                         levels = c("Transplantation",
                                    "Biotic interactions",
                                    "Interaction")))
#
delta_raw_gdd_fruit <- fruiting_onset_gdd_ambi |>
...
cooling_fruit_interaction_gdd_flower_nor_che <- ggplot(plot_df_gdd_fruit,
  ...
  labs(x = NULL,
       y = "Shifted onset (GDD)",
       color = "Condition")
cooling_fruit_interaction_gdd_flower_nor_che



# ggsave(filename = "Output/Onset/GDD_cooling_fruiting_onset_NOR_CHE.png", 
#        plot = cooling_fruit_interaction_gdd_flower_nor_che,

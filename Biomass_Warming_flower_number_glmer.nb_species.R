...
# Effect of warming on flower number glmer.nb adjusted for biomass -------------------------------------
# negative binomial model is better for our data

#  final figure of effect of cooling on flower number: p_bio_nb_species_warm ---------------------
...
m_flower_number_warm_bio_nb_species <- glmer.nb(
  value ~ treat_warming * treat_competition + pred_bio_centered +
    (1|species) + (1|block_ID),
  data = phenology_bio_species,
  subset = site == "high",
  control = glmerControl(optimizer = "bobyqa")
...
emm_fl_num_warm_bio_nb_species <- emmeans(
...
emm_fl_num_df_warm_bio_nb_species <- as.data.frame(
  summary(emm_fl_num_warm_bio_nb_species, infer = TRUE)
) |>
  rename(
    emmean = response,
    lower.CL = asymp.LCL,
...
# significances -----------------------------------------------------------
contr_warm_bio_nb_species <- contrast(emm_fl_num_warm_bio_nb_species, method = "pairwise") |>
  as.data.frame() |>
  mutate(type = "warming",
         sig = case_when(
           p.value < 0.001 ~ "***",
...
y_max <- max(emm_fl_num_df_warm_bio_nb_species$upper.CL, na.rm = TRUE)
spacing <- y_max * 0.27


warm_levels <- levels(emm_fl_num_df_warm_bio_nb_species$treat_warming)

ann_warm_nb_bio <- contr_warm_bio_nb_species |> 
  mutate(
    warm1 = warm_levels[1],
    warm2 = warm_levels[2],
...
ann_comp_nb_bio <- contr_comp_bio_nb_species |> 
  mutate(
    warm_x = as.numeric(factor(treat_warming, levels = warm_levels)),
    xmin = warm_x - offset,
    xmax = warm_x + offset,
    y = y_max + (nrow(ann_warm_nb_bio) + row_number()) * spacing,
    label = sig
  )
...
p_bio_nb_species_warm <- ggplot() +
  ...
  # EMM points
  geom_point(
    data = emm_fl_num_df_warm_bio_nb_species,
  ...
  geom_errorbar(
    data = emm_fl_num_df_warm_bio_nb_species,
    aes(x = treat_warming, ymin = lower.CL, ymax = upper.CL, color = treat_competition),
    width = 0.2,
    linewidth = 0.8,
    position = dodge
  ) +
  ...
  # warming brackets (within competition)
  bracket_geoms(ann_warm_nb_bio) +
  
  # competition brackets (within warming)
...
# ggsave(filename = "Output/Biomass/Warming_competition_flower_number_NOR_glmer.nb_adjusted_for_bimoass.png", 
#        plot = p_bio_nb_species_warm, 
